...
def best_rating_cuisines(df):
    df_aux = df.loc[:, ['first_cuisines','country_name','aggregate_rating']].groupby(['first_cuisines','country_name']).mean().sort_values('aggregate_rating', ascending=False).reset_index()                                                                                                   
    df_aux.columns=['Tipos_de_culinárias','Países', 'Média_notas_avaliações']
    df_aux = df_aux.head(10)
    fig = px.bar(df_aux, x='Tipos_de_culinárias', y='Média_notas_avaliações', title='Médias das notas das avaliações registradas por tipo de culinária', color='Países')
...
with st.container():
    ...
    with col2:
        ...
        col2.plotly_chart(best_rating_cuisines(df), use_container_width=True)
