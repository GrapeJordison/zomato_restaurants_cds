...
def best_rating_cuisines(df):
    df_aux = df.loc[:, ['first_cuisines','aggregate_rating','restaurant_id','votes']].groupby(['first_cuisines','aggregate_rating']).agg({'restaurant_id':['count'],'votes':['sum']}.sort_values('aggregate_rating', ascending=False).reset_index()
    df_aux = df_aux.head(10)
