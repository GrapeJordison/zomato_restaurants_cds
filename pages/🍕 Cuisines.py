...
def best_rating_cuisines(df):
    df_aux = df.loc[:, ['first_cuisines','aggregate_rating','restaurant_id']].groupby(['first_cuisines','aggregate_rating']).count().sort_values('aggregate_rating', ascending=False).reset_index()
    df_aux = df_aux.head(10)
    df_aux.columns=['Tipos_de_culinárias', 'Média_notas_avaliações','Quantidade_restaurantes']
